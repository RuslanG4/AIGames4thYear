...
*/
	...
	// formula is total = A/R^6 - B/R^12
	//B is repulsion , A is attraction
	int A = 100;
	int B = 600;

	float attractionDistance=50.f;
	/*float strength = 10.f;*/

	for (int i = 0; i < v.size(); i++)
	{
		position = position.subTwoVector(location, v[i].location);
		float distance = position.magnitude();
		if ((distance > 0) && (distance < neighbourDistance))
		{
			float distanceOverR = attractionDistance / distance;
			float r6 = std::pow(distanceOverR, 6);
			float r12 = std::pow(distanceOverR, 12);

			float force = ((A / r12) - (B / r6));

			position.normalize();
			position.mulScalar(force);
			sum.addVector(position);
		}
