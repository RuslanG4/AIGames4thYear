...
int main()
{
	...
	anchor.setSize(sf::Vector2f(50, 50));
	anchor.setOrigin(25, 25);
	...
	Formation wedge(60);
	wedge.anchor.setpostion(Vector2D(150.f, 150.f));
	wedge.anchor.setState(playerControlled, 1, 1);
	// wedge path
...
//	}
	...
	for (int i = 0; i < nagents; i++)
	{
		agent.push_back(Agent(100, 70, 5, 2));
...
//			Vector2D vel = Vector2D(targetStruct.position) - agent[i].getposition();
		...
		if (sf::Keyboard::isKeyPressed(sf::Keyboard::Left)) {
			wedge.anchor.decreaseRotation();
		}
		if (sf::Keyboard::isKeyPressed(sf::Keyboard::Right)) {
			wedge.anchor.increaseRotation();
		}
		if (sf::Keyboard::isKeyPressed(sf::Keyboard::Up)) {
			wedge.anchor.increaseSpeed();
		}
		if (sf::Keyboard::isKeyPressed(sf::Keyboard::Down)) {
			wedge.anchor.decreaseSpeed();
		}

		if (wedge.anchor.getState().st == playerControlled) {
			wedge.anchor.updatePlayerControlledMovement(dt.asMilliseconds());
			anchor.setRotation(wedge.anchor.getRotation());
		}
		//std::cout << wedge.getTarget().position.x << "\n";
